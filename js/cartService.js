...
function agregarAlCarrito(producto){
  let memoria = JSON.parse(localStorage.getItem("celus"));
  ...
  if(!memoria || memoria.length === 0) {
    ...
    const indiceProducto = memoria.findIndex(celu => celu.id === producto.id)
...
function actualizarNumeroCarrito(){
    let cuenta = 0;
    const memoria = JSON.parse(localStorage.getItem("celus"));
    if(memoria && memoria.length > 0){
